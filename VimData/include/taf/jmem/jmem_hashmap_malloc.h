...
/************************************************************************
重要说明:
1 使用方式与JceHashMapCompact略有不同，不需要指定chunk最小值、最大值和增长因子，只需要给出个数据长度大概均值;
2 与JceHashMapCompact相同，每个chunk额外消耗的字节数一样
3 与JceHashMapCompact不同，不在事先将内存分成大小不同的chunk，而是根据需要动态分配
4 每个chunk的大小最小是64字节(与实际数据块大小无关,实际数据块可以多个chunk拼接在一起)，实际可用的数据区长度最大为256G
************************************************************************/
