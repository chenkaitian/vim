...
/************************************************************************
重要说明:
1 使用方式与JceHashmap完全一致;
2 与JHashmap相比, 在64位os下面,每个chunk能够节约大概30个字节;
3 效率比JHashmap稍慢一点,可以忽略不计;
4 每个chunk的大小最大只能是64k(与实际数据块大小无关,实际数据块可以多个chunk拼接在一起)
************************************************************************/
 ...
 ***********************************************************************
 ...
 > 支持自动编解码, Key和Value的结构都通过jce2cpp生成;
 > jce协议支持自动扩展字段, 因此该hashmap支持自动扩展字段(Key和Value都必须是通过jce编码的);
 ...
 ***********************************************************************
 ...
 返回值说明: 
 > 注意函数所有int的返回值, 如无特别说明, 请参见TC_HashMapCompact::RT_
...
public:
    ...
    */
    ...
    public:
         ...
         *
         * @return int, 获取到数据, 返回:TC_HashMapCompact::RT_OK
         *              没有数据,返回:TC_HashMapCompact::RT_NO_DATA,
         *              系统默认GET,返回:TC_HashMapCompact::RT_NO_GET
         *              其他,则返回:TC_HashMapCompact::RT_LOAD_DATA_ERR
         */
    ...
    public:
    	 ...
    	 * 获取值
         * @return int
         *          TC_HashMapCompact::RT_OK:数据获取OK
         *          其他值, 异常
    	 */
    	int get(K& k)
        {
         ...
         * @return int
         *          TC_HashMapCompact::RT_OK:数据获取OK
         *          TC_HashMapCompact::RT_ONLY_KEY: key有效, v无效为空
         *          其他值, 异常
    	 */
    	int get(K& k, V& v)
        {
     ...
     * @param bDoClear: 是否清空
     * @return int
     *          TC_HashMapCompact::RT_DUMP_FILE_ERR: dump到文件出错
     *          TC_HashMapCompact::RT_OK: dump到文件成功
     */
     ...
     *
     * @return int
     *          TC_HashMapCompact::RT_LOAL_FILE_ERR: load出错
     *          TC_HashMapCompact::RT_VERSION_MISMATCH_ERR: 版本不一致
     *          TC_HashMapCompact::RT_OK: load成功
     */
     ...
     *
     * @return int
     *          TC_HashMapCompact::RT_NO_DATA: 没有当前数据
     *          TC_HashMapCompact::RT_ONLY_KEY:只有Key
     *          TC_HashMapCompact::RT_DIRTY_DATA: 是脏数据
     *          TC_HashMapCompact::RT_OK: 是干净数据
     *          其他返回值: 错误
     */
     ...
     *
     * @return int
     *          TC_HashMapCompact::RT_READONLY: 只读
     *          TC_HashMapCompact::RT_NO_DATA: 没有当前数据
     *          TC_HashMapCompact::RT_ONLY_KEY:只有Key
     *          TC_HashMapCompact::RT_OK: 设置成功
     *          其他返回值: 错误
     */
     ...
     *          TC_HashMapCompact::RT_NO_DATA: 没有当前数据
     *          TC_HashMapCompact::RT_ONLY_KEY:只有Key
     *          TC_HashMapCompact::RT_OK: 设置脏数据成功
     *          其他返回值: 错误
     */
     ...
     *
     * @return int:
     *          TC_HashMapCompact::RT_NO_DATA: 没有数据
     *          TC_HashMapCompact::RT_READONLY: 只读模式
     *          TC_HashMapCompact::RT_ONLY_KEY:只有Key
     *          TC_HashMapCompact::RT_OK:获取数据成功
     *          TC_HashMapCompact::RT_LOAD_DATA_ERR: load数据失败
     *          其他返回值: 错误
     */
     ...
     * @param bDirty: 是否是脏数据
     * @return int:
     *          TC_HashMapCompact::RT_READONLY: map只读
     *          TC_HashMapCompact::RT_NO_MEMORY: 没有空间(不淘汰数据情况下会出现)
     *          TC_HashMapCompact::RT_OK: 设置成功
     *          其他返回值: 错误
     */
     ...
     *          TC_HashMapCompact::RT_READONLY: map只读
     *          TC_HashMapCompact::RT_NO_DATA: 没有当前数据
     *          TC_HashMapCompact::RT_ONLY_KEY:只有Key, 也删除了
     *          TC_HashMapCompact::RT_OK: 删除数据成功
     *          其他返回值: 错误
     */
    int del(const K& k)
    {
