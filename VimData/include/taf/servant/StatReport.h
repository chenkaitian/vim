...
#include "servant/PropertyPlus.h"
...
public:
	...
	typedef  map<string, map<string, int> >			MapDynamicWeightMsg;
	
	struct DynamicWeightDataStat
	{
		int64_t iTime;
		string	sObj;
		string	sIp;
	...
	typedef  TC_LoopQueue<DynamicWeightDataStat*>         dynamic_weight_queue;
...
public:
    ...
    void setReportInfo(const StatFPrx& statPrx,
                       const PropertyFPrx& propertyPrx,
                       const PropertyPlushFPrx &propertyPlusPrx,
                       const string& strModuleName,
                       const string& strModuleIp,
                       const string& strSetDivision,
                       int iReportInterval = 60,
    ...
    void report(const string& strMasterName,
                ...
                int  iSptime,
                int  iReturnValue = 0,
				string sSlaveContainer = "",
				string sMasterContainer = "");
	...
	* @param sAppServer
	*
	* @return PropertyPlusPtr
	*/
   PropertyPlusPtr getPropertyPlusReport(const string &sPropertyName, const string &sAppServer="");
    ...
    PropertyPlusPtr createPropertyPlusReport( const string &sPropertyName, const string &sAppServer="");

public:
    void report(size_t iSeq,MapStatMicMsg * pmStatMicMsg);

	void reportDynamicWeightMsg(size_t iSeq,DynamicWeightDataStat * pDynamicWeightMsg);
...
public:
    ...
    */
     ...
     * @return void
     */
	void filterResultValue(map<taf::Int64, taf::Int32> &mResultValue);
    /**
     * 结束线程
     */
    void terminate();
...
private:
	...
	//统计动态权重的信息
	void setDynamicWeighMsg();

public:
	//获取动态权重信息
	int getDynamicWeightMsg(string &sResult);
private:
    time_t              _tTime;
    ...
    PropertyPlushFPrx _propertyPlusPrx;

    map<string, PropertyPlusPtr>          _mPropertyPlusMsg;

	string _sContainerName;		// 主调上报，表示主调容器名，被调上报，表示被调容器名
...
private:
	...
	vector<dynamic_weight_queue*> vDynamicWeightMsg;
	TC_ReadersWriterData<MapDynamicWeightMsg> dynamicWeightCache;
	TC_ReadersWriterData<int64_t> timeCache;
