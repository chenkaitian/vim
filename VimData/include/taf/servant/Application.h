...
#define TAF_CMD_SET_DAYLOG_LEVEL       "taf.enabledaylog"      //设置按天日志是否输出: taf.enabledaylog [remote|local]|[logname]|[true|false]

#define TAF_CMD_CLOSE_CORE         "taf.closecore"          //设置服务的core limit:  taf.setlimit [yes|no]
#define TAF_CMD_SET_MEMLOG         "taf.memlog"             //设置服务是否使用内存日志:  taf.memlog [true|false]
#define TAF_CMD_SEGV_CORE_DUMP	   "taf.segvcoredump"		//设置服务是否进行segv coredump:  taf.segvcoredump [true|false]
#define TAF_CMD_VIEW_DYNAMIC_WEIGHT    "taf.dynamicweight"  //查看服务作为客户端的动态权重状态
#define TAF_CMD_RELOAD_LOCATOR     "taf.reloadlocator"
#define FRAMEWORK_GRAY             "taf.framework.GrayFlag"
/**
...
struct ServerConfig
{
	...
	static int		   IsMemLog;			//是否打开内存日志，0，不打开，1，打开
	...
	static int         OpenFangZhou;
	static int         AuthType;            // 鉴权类型：本地、远程、无
    static std::string AuthLocator;         // 鉴权服务地址
};
...
protected:
	...
	bool cmdSetMemLog(const string& command, const string& params, string& result);

	bool cmdSegvCoreDump(const string& command, const string& params, string& result);
	...
	bool cmdDynamicWeight(const string& command, const string& params, string& result);
	/*
...
protected:
     ...
     * @param servant
     */
    void addServantAuthProcess(const string& servant, const TC_EpollServer::auth_process_functor& protocol);
