...
/**
 * 定义鉴权的打包函数
 * 鉴权所需信息:AK SK, servant name
 * 注意必须是线程安全的或是可以重入的
 */
//int/*ok or error*/ serialize_auth_req(int authState, const BasicAuthInfo& info, string& out)
struct BasicAuthInfo;
typedef TC_Functor<int, TL::TLMaker<int, const BasicAuthInfo&, string&  >::Result> auth_req_functor;

/**
 * 定义鉴权的响应函数
 * 注意必须是线程安全的或是可以重入的
 */
// int/*new state*/ auth_rsp(int currentState, ResponsePacket& rsp, string& out/*data for new state*/) 
typedef TC_Functor<int, TL::TLMaker<int, const ResponsePacket&, string& >::Result> auth_rsp_functor;
