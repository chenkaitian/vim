...
public:
     ...
     * 设置coro并行请求的共享智能指针
     */
	virtual void setCoroParallelBasePtr(taf::CoroParallelBasePtr pPtr) { _pPtr = pPtr; }

	/**
     * 获取coro并行请求的共享智能指针
     */
	virtual const taf::CoroParallelBasePtr& getCoroParallelBasePtr() { return _pPtr; }
...
public:
	...
	/**
     * 透传主调名称信息，目前透传给Dcache
     */
	static string TAF_MASTER_KEY; 
...
public:
    ...
    */
	...
	/**
    *设置成功率和平均耗时在计算动态权重的比例
    */
    void taf_set_weight_ratio(size_t iSuccRatio, size_t iAvgTimeRatio);

	/**
     * 设置超时检查参数
     */
    void taf_set_check_timeout(const CheckTimeoutInfo& checkTimeoutInfo);

	/**
     * 获取超时检查参数
     */
	CheckTimeoutInfo taf_get_check_timeout();
     ...
     * 一致性hash方法
     */
    virtual ServantProxy* taf_consistent_hash(int64_t key);
	 ...
	 */
	virtual void taf_setMasterFlag(bool bMasterFlag) {_bMasterFlag = bMasterFlag;}

	/**
	 * 设置鉴权函数
	 */
	void taf_setAuthReqFunc(const auth_req_functor& f);
	/**
	 * 设置鉴权响应函数
	 */
	void taf_setAuthRspFunc(const auth_rsp_functor& f);
...
private:
	...
	//每个业务线程与客户端网络线程之间的请求队列大小
	int _queueSize;

	//最小的超时时间
	int64_t _iMinTimeout;
};
