...
static const string TIME_OVER_FLAG = "FZMD";
static const string SIZE_OVER_FLAG = "FZMDO";
static const string SINGLE_OVER_FLAG = "FZMDS";
namespace taf
{
/////////////////////////////////////////////////
/**
 ...
 */
	...
	public:
		void operator()(ostream &of, const deque<pair<int, string> > &ds)
		{
			...
			while (it != ds.end())
			{
				if (it->second != TIME_OVER_FLAG&& it->second != SIZE_OVER_FLAG&&it->second != SINGLE_OVER_FLAG)
				{
					of << it->second;
				}
				++it;
 ...
 */
	...
	public:
		...
		*/
		...
		//bool getrefresh(){ return _refresh; }
	protected:
 ...
 */
	...
	protected:
		...
		void head(char *c, int len, int level)
		{
			...
			struct timeval mt;
			TC_TimeProvider::getInstance()->getNow(&mt);
			tm tt;
			localtime_r(&mt.tv_sec, &tt);
			time_t t=mt.tv_sec;
			if (hasFlag(TC_Logger::HAS_TAG))
			{
			//	n += snprintf(c + n, len - n, "%s%s", "FZMD", _sSepar.c_str());
				//time_t t = TNOW;
			...
			if (hasFlag(TC_Logger::HAS_IP))
			{
			//	_buffer.setrefresh(true);
				n += snprintf(c + n, len - n, "%s%s",_ip.c_str(), _sSepar.c_str());
			}
			...
			if (hasFlag(TC_Logger::HAS_MTIME))
			{
				...
				n += snprintf(c + n, len-n, szFormat,
							  tt.tm_year + 1900, tt.tm_mon+1, tt.tm_mday, tt.tm_hour, tt.tm_min, tt.tm_sec, mt.tv_usec/1000,_sSepar.c_str());
			}
...
////////////////////////////////////////////////////////////////////////////////
	...
	public:
		...
		size_t getBufferSize() { return _roll->getBufferSize();}
		
		void activeFlush(deque<pair<int, string> >&buffer)  { _roll->activeFlush(buffer);}
    ...
    */
	...
	public:
		...
		public:
			 ...
			 */
			void init(const string &path, const string &format = "%Y%m%d",bool bHasSufix=true,const string&sConcatstr="_",const TafLogTypePtr& logTypePtr=NULL,bool bIsRemote=false,bool bFangzhou=false)
			{
				this->_roll->init(path, format, bHasSufix, sConcatstr, logTypePtr, bIsRemote, bFangzhou);
			}
		    ...
		    void setFangZhou(bool en){ this->_roll->setFangZhou(en); }
		};

		/**
		 * @brief 构造
		 */
		TC_RollByTime() : _lt(TNOW), _logTypePtr(NULL), _bRemoteType(false), _bFangZhou(false)
		{
			_bHasSufix = true;
			_sConcatStr = "_";
		}
		...
		void init(const string &path, const string &format = "%Y%m%d", bool bHasSufix = true, const string &sConcatstr = "_", const TafLogTypePtr& logTypePtr = NULL, bool bIsRemote = false, bool bFangZhou
			=false)
		{
			TC_LockT<TC_ThreadMutex> lock(*this);
		...
		void setFormat(const string &format){ TC_LockT<TC_ThreadMutex> lock(*this); _format = format;}
			 ...
			 /**
			   * 如果是设置打印到方舟，则不能生成本地文件
			   *
			   */
			 void setFangZhou(bool en){ TC_LockT<TC_ThreadMutex> lock(*this); _bFangZhou = en; }
		/**
		 * @brief 获取写示例.
		 *
		 * @return WriteT&
