...
public:
     ...
     */
	TC_Endpoint(const string& host, int port, int timeout, int istcp = true, int grid = 0, int qos = 0, int status = 0, int weight = -1, unsigned int weighttype = 0, int cpuload = -1, int64_t sampletime = 0, int weightflow = -1)
    {
		init(host, port, timeout, istcp, grid, qos, status, weight, weighttype, cpuload, sampletime, weightflow);
    }

	TC_Endpoint(const string& host, int port, int timeout, string sContainer, int istcp = true, int grid = 0, int qos = 0, int status = 0, int weight = -1, unsigned int weighttype = 0, int cpuload = -1, int64_t sampletime = 0, int weightflow = -1)
	{
    ...
    TC_Endpoint(const TC_Endpoint& l)
    {
        _host   = l._host;
        _port   = l._port;
		_bindHost = l._bindHost;
		_managePort = l._managePort;
        _timeout= l._timeout;
        _istcp  = l._istcp;
        _grid   = l._grid;
        _qos    = l._qos;
		...
		_cpuload = l._cpuload;
		_sampletime = l._sampletime;
		_weightflow = l._weightflow;
		_sContainer = l._sContainer;
    }
    ...
    {
        return (_host == l._host && _port == l._port && _timeout == l._timeout && _istcp == l._istcp && _grid == l._grid && _qos == l._qos && _status == l._status && _weight == l._weight && _weighttype == l._weighttype && _cpuload == l._cpuload && _sampletime == l._sampletime && _weightflow == l._weightflow && _sContainer == l._sContainer);
    }
     ...
     * @param port
     */
	string getBindHost() const          { return _bindHost;  }
	...
	*/
	...
	int getManagePort() const            { return _managePort; }
    /**
     * @brief 设置超时时间
     * @param timeout
     */
	...
	*/
    ...
    string toString()
    {
		...
		if (!_bindHost.empty()) os << " -b " << _bindHost;
		if (_managePort != 0) os << " -a " << _managePort;
        return os.str();
    }
...
private:
	void init(const string& host, int port, int timeout, int istcp, int grid, int qos, int status, int weight, unsigned int weighttype, int cpuload, int64_t sampletime, int weightflow);
