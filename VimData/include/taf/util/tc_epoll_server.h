...
public:
	 ...
	 * 注意必须是线程安全的或是可以重入的
	 */
    typedef TC_Functor<int/*new state*/, TL::TLMaker<int /*current state*/, const RequestPacket&, std::string&>::Result> auth_process_functor;
	/**
	 * 定义协议解析接口的操作对象
	 * 注意必须是线程安全的或是可以重入的
	 */
	...
	protected:
     ...
     //bool processAuth(Connection* conn, const string& requestData);     
	...
	public:
		 ...
		 * @param af
		 */
		void setAuthProcessFunc(const auth_process_functor& af) { _authf = af; }

		/**
		 * 获取鉴权函数
		 * @return auth_process_functor
		 */
		auth_process_functor& getAuthProcessFunc() { return _authf; }
		/**
		 * 设置AK+SK
		 */
		void setAuthKeys(const string& accKey, const string& hashSecret) { _accessKey = accKey, _hashSecretKey = hashSecret; }
		/**
		 * 获取AK
		 */
		 ...
		 * 获取SK
		 */
		const string& getHashSecretKey() const  { return _hashSecretKey; }
		/**
		 * 注册鉴权包裹函数
		 * @param apwf
		 */
