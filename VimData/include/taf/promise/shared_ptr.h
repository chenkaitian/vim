#ifndef __TAF_PROMISE_SHARED_PTR_H__
#define __TAF_PROMISE_SHARED_PTR_H__
...
namespace promise {
    ...
    namespace detail 
	{
        ...
        inline void sp_enable_shared_from_this(const promise::SharedPtr<X> *pp,
                                               const promise::EnableSharedFromThis<T> * px)
        {
            if (px != NULL) 
			{
                px->acceptOwner(*pp);
    ...
    public:
        ...
        ~SharedPtr()
        {
            PROMISE_ASSERT((m_px && m_pn && m_pn->use_count() > 0)
                       || (m_px == NULL && m_pn == NULL));
